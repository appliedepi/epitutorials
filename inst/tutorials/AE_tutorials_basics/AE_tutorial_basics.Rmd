...
knitr::include_graphics("images/support-help-moz.png")
```
...
```{r viz, fig.margin = TRUE, echo = FALSE, out.width = "75%", out.height="100%", fig.cap = ""}
...
::: {}
```{r dashboards, fig.margin = TRUE, echo = FALSE, out.width = "45%", out.height="100%", fig.cap = "", fig.show='hold'}
...
::: {}
```{r shiny, fig.margin = TRUE, echo = FALSE, out.width = "45%", out.height="100%", fig.cap = "", fig.show='hold'}
...
What can we do with a *character* vector We cannot multiply it by 2 because it is not numeric... for fun, **put this vector into another function**: `toupper()`, which changes all of the characters to upper case. We have set this code box to remember what you wrote in the previous codebox, so you can reference the `jurisdictions` object that you created.  
...
What are some possible arguments for this function? **Write your answer within "quotation marks" and press 'Submit answer'.** This question will not be graded, it is simply to make you think.

```{r oven-bake-free-text, exercise=TRUE, exercise.lines = 5}
"Write some free text here in the quotation marks.
What (object) needs to go into the oven?
What oven settings must be adjusted?
What other instructions must be followed?"
```

```{r oven-bake-free-text-check}
grade_this({
  # custom checking code appears here
  if (is.character(.result)) {
    pass("Thank you for trying this mind exercise.")
  }
  fail("Put your answer within quotation marks.")
})
```
...
Now use the codebox below to try the `paste()` function and its `collapse=` argument on your object `jurisdictions` (press the button to bring in your old command).  
...
In this tutorial, you can run a command with the question mark from any codebox to launch the function documentation in a new window, for example: `?max`. Try it below:  

```{r help-doc, exercise = TRUE, exercise.lines = 2}
...
Revise the file names to align with naming best-practices. **Leave the names in quotation marks**:  

```{r rename-central, exercise=TRUE, exercise.lines = 2}
"Central Hospital 12012014.csv"
```

```{r rename-central-check}
grade_this({
  # custom checking code appears here
  if (is.character(.result)) {
    pass("Thanks for your answer. We suggest: 20141201_hosp_central.csv")
  }
  fail("Ensure your answer is within quote marks to be correctly submitted")
})
```


```{r rename-military, exercise=TRUE, exercise.lines = 2}
"Dec 1 2014 Military Hospital.csv"
```

```{r rename-military-check}
grade_this({
  # custom checking code appears here
  if (is.character(.result)) {
    pass("Thanks for your answer. We suggest: 20141201_hosp_central.csv")
...
```{r rename-port, exercise=TRUE, exercise.lines = 2}
...
grade_this({
  # custom checking code appears here
  if (is.character(.result)) {
    pass("Thanks for your answer. We suggest: 20141201_hosp_port.csv")
  }
  fail("Ensure your answer is within quote marks to be correctly submitted")
})
```
...
We suggest completing our tutorial on "Tidy data" next.  
