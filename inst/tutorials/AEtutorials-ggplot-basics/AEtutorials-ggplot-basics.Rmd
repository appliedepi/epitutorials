...
"Aesthetics" are features whose display could vary or each data point. An obvious example are the X and Y co-ordinates - for example, if we "map" the column `age` to the X-axis, then every row in the data will be evaluated and may be placed on a different X-axis position. But aesthetics can also include the color, shape and other properties of the plot that we will explore further in the tutorial.  
...
The `mapping = ` argument of `ggplot()` is atypical in that you must provide the values to it **within the `aes()` function.**  

> `mapping = aes(x = COLUMN, y = COLUMN)`  

Remember that this code above is occurring *within* the `ggplot()` function. The `mapping = aes()` itself IS the second expected argument to `ggplot()`. Make the column mapping assignments *within* the `aes()` function.  

```{r aes, echo=T, eval=T}
ggplot(data = linelist, mapping = aes(x = age, y = ht_cm))
```

Above, we have set the axes X and Y to the age and height columns from the linelist data. You can see this represented on the axes scales above, which now have titles and values. Note that no data is yet displayed.  

The first 25 rows of the linelist data are reproduced below. Try changing the Y axis to `temp` rather than `ht_cm` in the **R** window below. You can use the arrows to scroll left and right through the table and find the correct column. 
...
### A note on newlines and indentations  
...
Note how the third more vertical coding approach looks similar to a "table of contents" and allows explanatory comments to by typed to the left of each specific component of the command.  
