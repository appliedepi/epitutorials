...
* Creating spreadsheets that are "machine-readable"  
* Identify data entry pitfalls that will result in difficulty cleaning and analyzing data in R  
...
### **A "linelist" of cases in a fictional Ebola outbreak**
...
This dataset contains `r nrow(linelist)` rows and `r ncol(linelist)` columns. Below are the first 25 rows:  
...
We will review a few example Excel spreadsheets.  

One contains information about "coverage" at a number of sites in May and June 2022:  
...
```{r, out.width = "25%", fig.align = "center", echo=F}
...
question_text(
  "What are some features of your data that make it particularly difficult to clean for analysis?",
...
* Consistency in data structure, consistency in tools, and efficient analyses  
* Placing variables in columns allows may R function to easily handle your data (e.g. `ggplot() for plotting) - the packages from the {tidyverse} are designed to work with tidy data  
...
DT::datatable(long_example, rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
...
DT::datatable(long_example_pivoted, rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
...
question_text(
  ...
  answer_fn(function(value) {
    if (grepl(paste(1:9, collapse = "|"), value)) {
      correct("Thank you for reflecting on this.")
    }
  }, )
)
```
...
```{r, out.width = "45%", fig.align = "center", echo=F, fig.cap="R has an initial learning curve, but more complex tasks become much easier than in Excel."}
...
The empty rows from row number 4 onwards are also an issue, as they actually imply the value that is given in the cell above, but will be recorded as `NA` values. This will require significant extra cleaning.  
...
Here are the first 5 rows of the `linelist` data frame:  
...
*   Do not use spaces, but rather underscores (`_`) in the column names  
...
Congratulations! You have completed the Data preparation tutorial in this series!  

We suggest completing our tutorial on "Data cleaning" next.  
