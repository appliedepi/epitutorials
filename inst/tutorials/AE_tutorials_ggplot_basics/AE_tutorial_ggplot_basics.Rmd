...
linelist <- rio::import(system.file("dat/tutorial_linelist_cleaned.rds", package = "epitutorials"))
...
* **This tutorial teaches**: Introduction to visualization of data using {ggplot2} and public health examples  
...
Below is an command that imports the file "linelist.rds", which is located in the RStudio project's "data" folder.  
...
linelist_raw <- import(here("data", "linelist.rds"))
```
...
Below is an R exercise where you can edit, write, and run your own code. Try editing the code such that the y-axis reflects temperature rather than height. Clicking "Run code" will run what you have entered and produce an output. To check your answer against the solution click "Submit Answer". If you need help, click the "Hints" button above the code chunk to get a hint to the solution. You can also ask a question in our [Community Forum](www.community.appliedepi.org).  

The first 25 rows of the linelist data are reproduced below. You can use the arrows to scroll left and right through this table and find the correct column.  
...
As with the other aesthetics, we have seen that `shape` can be dynamic. Below the point `shape` is assigned to the column `gender`, and the color to `age_cat`. Note the two legends that appear on the right.  
...
ggplot(
  ...
  mapping = aes(
    ...
    color = age_cat)) +  # color by age category
geom_point(
  size = 3,              # all points are size 3
  alpha = 0.3)           # all points are mostly transparent
```

Below the points are all assigned to shape 21 - the circle with both an exterior `color` and an interior `fill`. Below we assign the color as "black" and the fill to the column `age_cat`.  
...
ggplot(
  ...
  mapping = aes(
    x = age_years,
    y = wt_kg,
    fill = age_cat)) +   # interior fill of points by age category
geom_point(
...
Below, we create facets the case definition (the column `case_def`). Note that we write a "~" before the column name we want to facet by, otherwise we will get an error message. This symbol can be interpreted as the word "by".  
...
Try editing the below code, which facets the histogram by the column `hospital`:

1. Add a "free_y" free y axis
2. Have only 2 columns of plots
3. Assign `fill` to the column `case_def` (tip: review the aesthetic mapping section above)  
...
```{r facet-test-check}
grade_this_code("correct, we remove the scales argument, change ncol to 2 and add fill = case_def to the mapping = aes() of the histogram")
```

```{r facet-test-hint}
1. Add the argument scales = "free_y" to facet_wrap()
2. Adust ncol = 
3. We want to specify fill = case_def inside the aes() mapping for the histogram
```
...
ggplot(
  data = linelist,
  mapping = aes(x = date_onset)) +
geom_histogram(mapping = aes(fill = case_def)) +
facet_wrap(
  ~hospital,
  scales = "free_y",
  ncol = 2)
...
linelist %>% 
   drop_na(gender, hospital) %>%        # drop rows that are missing (NA) gender and hospital 
   filter(hospital != "Missing") %>%    # drop rows where hospital is "Missing" 

# pipe filtered linelist into the ggplot() command
# note: no data= argument needed
ggplot(mapping = aes(x = date_onset, fill = case_def)) +
geom_histogram() +
facet_grid(hospital ~ gender)
```
...
quiz(
  question_text("When would you use a facet rather than an aesthetic?",
    ...
    answer("Facet grid is most useful for 2 discrete variables, where you are see all or most combinations of these two variables in your data. To split by one variable with a lot of levels or just to provide an additional aesthetic without cluttering the plot facet_wrap may be more useful.", correct = T), incorrect = "Facet grid is most useful for 2 discrete variables, where you are see all or most combinations of these two variables in your data. To split by one variable with a lot of levels or just to provide an additional aesthetic without cluttering the plot facet_wrap may be more useful. "
    )
...
For practice, try setting the date axis breaks in this plot to appear every 2 months:
...
Conveniently, we can simply write `labels = percent`, and ggplot will know to use this function (assuming you have installed and loaded the {scales} package).  
...
facet_wrap(~case_def,                     # wrap by case_def as we have already used color for gender
           ncol = 1) +                     # one column as we are dealing with wide rather than tall plots
labs(
  title = "Cases by case definition",
  subtitle = str_glue(
