...
You can practice loading them in the codebox below. If you are unfamiliar with this `pacman::p_load()` command, review the earlier tutorial on R basic syntax. In brief, this command will automatically install R packages if they are not already installed, *and* load them for use.  
...
linelist_raw <- import(here("data", "linelist.xlsx"))   # import data and save as named object
```
...
We begin by looking at the dataset using the `names()` function:  

```{r, echo = T, eval = T}
names(linelist_raw)
```

We can see that there are numerous violations of this clean syntax. We have names with spaces (`onset date`), different naming patterns for dates (`onset date` vs `date of report`), and names with special characters (`wt (kg)`).  
...
######################################################
surv <- surv_raw %>%        # create new "clean" dataset, starting with the raw
...
We begin the cleaning pipe chain like this:    

1) Create a name for your "clean" data frame, e.g. `linelist`  

```{r, echo=TRUE, eval=FALSE}
linelist
```

2) Use the assignment operator `<-` to assign it the value of the "raw" data frame (`linelist_raw`)  

```{r, echo=TRUE, eval=FALSE}
linelist <- linelist_raw
```

3) Add a pipe to the right side, and the first cleaning function below
...
The placement of new lines and spaces in R does not impact the code execution, but can impact how easy it is to read the code. The default spacing will look more like this:  
...
Another way to write the same command, is to pipe `surv_raw` into the function `names()`.  
...
To finish cleaning our column names, we also want similar columns to have similar style nomenclature. For example, the date columns should all begin with **date_** so they sort and are easily referencable together in later commands.  
...
The `rename()` function allows you to specify a NEW column name, and the old column name with the syntax `NEW = OLD`. Multiple changes can be separated by commas, within the `recode()` parentheses.  

Below, `rename()` is added to the pipe chain after `clean_names()` (with a pipe operator `%>%`). The column name `onset_date` is changed to `date_onset`, and the column `date_of_report` is changed to `date_report`.  
