...
description: >
  Learn how to clean a public health dataset using R
---
...
This is the **third** tutorial in the series.  
...
After completing this tutorial, we recommend you complete our tutorial on "Data visualization basics".  
...
You can read more in the previous tutorial and in the [Epi R Handbook](https://epiRhandbook.com/en/import-and-export.html) about how RStudio projects, the {here} package, and the {rio} package can be efficiently used together to make the data import process very easy!
...
quiz(caption = "Quiz - reviewing the data",
  question("How many columns are there in the data frame?",
    allow_retry = T,
    answer(nrow(linelist_raw), message = "No, this is the number of rows/observations"),
    answer(ncol(linelist_raw), correct = TRUE, message = "Yes this is correct!"),
  ...
  question("Review the information on character columns. What is the number of missing values in the hospital column?",
    allow_retry = T,
    answer(sum(is.na(linelist_raw$hospital)), correct = TRUE),
...
You will learn more about the `tabyl()` function and its utilities in a later tutorial - it is quite useful and versatile!  

(*and yes, we know that it says "percent" by default in the headings when it really shows a decimal proportion - this discordance is fixed when you use the funtion further*)  
...
The first function in our pipe chain is typically the `clean_names()` function from the {janitor} package. This function does much of the hard work of cleaning column names for us!  
...
**In the codebox below, begin your pipe chain by piping the `linelist_raw` data frame to the `clean_names()` function, storing the result as `linelist`.** Don't forget that you can use Ctrl + Shift + m as a shortcut to write the pipe operator.  
...
**Do not expect any output** because you ran an *assignment* command. But now that you created the cleaned `linelist` you can inspect its column names using `names()` 
...
Compare these names to the previous ones above. You can see that this has done a lot of the work for us, replacing spaces with "_", and ensuring there are no unusual characters (see the onset date, weight, and height columns).
...
```{r rename, exercise=TRUE, exercise.lines = 8}
linelist <- linelist_raw %>% 
  clean_names()
```
...
linelist %>% 
  select(case_id, age, gender, lat, lon) %>% 
  filter(age > 65)
```
...
Use a minus symbol ( - ) to drop a column by name, and all other columns are retained.  
...
```{r mutate-wt, exercise=TRUE, exercise.lines = 10}
linelist <- linelist_raw %>% 
  clean_names() %>% 
  rename(date_onset = onset_date,
         date_report = date_of_report,
...
Then, within a `mutate()` command, we suggest that you use one of the 3 {lubridate} functions below to transform the column to class "Date".  
...
```{r mutate-date, exercise=TRUE, exercise.lines = 12}
linelist <- linelist_raw %>% 
  clean_names() %>% 
  rename(date_onset = onset_date,
         date_report = date_of_report,
...
As you can see, `r nrow(linelist_raw) - nrow(demo)` rows were dropped from the original `r nrow(linelist_raw)`.  
...
```{r recode, exercise=TRUE, exercise.lines = 17}
linelist <- linelist_raw %>% 
  clean_names() %>% 
  rename(date_onset = onset_date,
         date_report = date_of_report,
...
`r fontawesome::fa("exclamation", fill = "red")` Please note that missing values (`NA`) are removed by default unless you specify that they should be kept!  

For example, the command below will only keep rows with a `date_onset` prior to 15 April 2020, and any row with a missing date of onset will also be dropped.  

```{r eval=FALSE, echo=TRUE}
linelist %>% 
  filter(date_onset < ymd("2020-04-15"))
```

To keep the missing values, you would need to add an "OR" statement like this:  

```{r eval=FALSE, echo=TRUE}
linelist %>% 
  filter(date_onset < ymd("2020-04-15") | is.na(date_onset))
```

The same principle applies for numeric filters, character filters, etc. Remember that in the filter you are specifying which rows to *keep*.  
...
If you have extra time, consider completing the extra exercises below. Otherwise, we welcome you to advance to the next tutorial in the series.  
...
quiz(caption = "Quiz - Western districts",
  question("How many cases were there in western districts?",
    allow_retry = TRUE,
    answer("170"),
    answer(western_cases_n, correct = TRUE),
    answer("418")
  )
)
```
